...
// XOR block decryption (identica all'encryption)
void decrypt_block(uint64_t* dest, uint64_t src, uint64_t key) {
    *dest = src ^ key;
}

// Divide i dati in blocchi da 8 byte (64 bit), con padding se necessario
...
// Converte i blocchi da uint64_t in un buffer byte*, rimuovendo il padding
